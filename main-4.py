...
ADMIN_COMMANDS_HELP = """
🗺 <b>Карты и билды</b>
/addmap — добавить/обновить карту (название на первой строке, дальше — пики)
...
async def reply(event, text, **kwargs):
    """Отправляет сообщение как ответ на Message или CallbackQuery и
    запоминает id отправленного сообщения для последующей автоочистки чата."""
...
def _extract_name_and_content_html(message: Message, command: str):
    """Достаёт название (первая строка, как обычный текст — для поиска) и
    контент (всё остальное, как HTML — чтобы сохранить форматирование и
    платные эмодзи), из сообщения вида "/addmap Название\\nКонтент"."""
...
async def cmd_start(message: Message):
    ...
    sent = await reply(message, text, reply_markup=await main_menu_kb())
...
async def cb_referrals(callback: CallbackQuery):
    ...
    await reply(callback, template.format(link=link, count=count))
    await callback.answer()
...
async def cmd_addmap(message: Message):
    name, content_html = _extract_name_and_content_html(message, "/addmap")
    ...
    await set_pending_action(
        message.from_user.id, "addmap", {"name": name, "content_html": content_html}
    )
    await reply(
        message,
...
async def cmd_addbuild(message: Message):
    name, content_html = _extract_name_and_content_html(message, "/addbuild")
    if not name or not content_html:
        await reply(
            message,
            "Пришли одним сообщением:\nНазвание бойца\nдальше — билд "
        ...
        return
    await set_pending_action(
        message.from_user.id, "addbuild", {"name": name, "content_html": content_html}
    )
    await reply(
        message,
...
async def _apply_addmap(admin_id, payload):
    await add_entry("maps", payload["name"], payload["content_html"])
    return f"✅ Карта «{payload['name']}» опубликована."


@register_apply("addbuild")
async def _apply_addbuild(admin_id, payload):
    await add_entry("builds", payload["name"], payload["content_html"])
...
async def cb_actconfirm(callback: CallbackQuery):
    ...
    if not pending:
        await callback.message.edit_text("Нечего подтверждать — запрос устарел.")
        await callback.answer()
        return
...
def _make_editable_handlers(key: str, command_name: str, prompt: str, action_type: str, label: str):
    ...
    async def receive_handler(message: Message):
        await clear_pending_input(message.from_user.id)
        if not message.text:
            await reply(message, f"Нужен текст. Попробуй снова: /{command_name}")
            return
        await set_pending_action(message.from_user.id, action_type, {"key": key, "value": message.text})
        await reply(
            message,
...
for _key, (_cmd, _prompt, _default) in EDITABLE_TEXTS.items():
    _make_editable_handlers(_key, _cmd, _prompt, "settext", "текст")

for _key, (_cmd, _prompt, _default) in BUTTON_LABELS.items():
    _make_editable_handlers(_key, _cmd, _prompt, "setbtn", "кнопка")
...
async def cb_subscription_status(callback: CallbackQuery):
    ...
    await reply(callback, text, reply_markup=buy_kb())
    await callback.answer()
...
async def process_successful_payment(message: Message):
    ...
    await reply(message, template.format(days=SUBSCRIPTION_DAYS))
...
async def handle_search(message: Message):
    ...
    if not await has_access(user_id):
        text = await get_setting("no_access_text")
        await reply(message, text, reply_markup=buy_kb())
        return
    ...
    if map_entry:
        ...
        await reply(message, map_entry["content"])
    if build_entry:
        await log_query("build", build_entry["name"])
        await reply(message, build_entry["content"])
