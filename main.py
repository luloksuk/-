...
async def cmd_start(message: Message):
    ...
    sent = await reply(message, text, reply_markup=main_menu_kb())
...
async def cmd_addmap(message: Message):
    ...
    if not raw:
        await reply(message, "Пришли одним сообщением:\nНазвание карты\nдальше — пики (любой текст)")
        return
    name, content = _split_first_line(raw)
    if not name or not content:
        await reply(message, "Нужно название карты на первой строке и содержимое дальше.")
        return
    await add_entry("maps", name, content)
    await reply(message, f"✅ Карта «{name}» сохранена.")
...
async def cmd_addbuild(message: Message):
    ...
    name, content = _split_first_line(raw)
    if not name or not content:
        await reply(message, "Нужно название бойца на первой строке и содержимое дальше.")
        return
    await add_entry("builds", name, content)
    await reply(message, f"✅ Билд на «{name}» сохранён.")
...
async def cmd_delmap(message: Message):
    ...
    await reply(message, f"Удалить карту «{name}»?", reply_markup=confirm_kb("delete"))
...
async def cmd_delbuild(message: Message):
    ...
    await reply(message, f"Удалить билд на «{name}»?", reply_markup=confirm_kb("delete"))
...
async def cmd_addsub(message: Message):
    ...
    await reply(message, f"✅ Подписка для {user_id} продлена до {new_until[:16]} UTC")
...
async def cmd_delsub(message: Message):
    ...
    await reply(message, f"✅ Подписка для {args[0]} снята")
...
async def cmd_addadmin(message: Message):
    ...
    await reply(message, f"✅ {args[0]} теперь админ")
...
async def cmd_deladmin(message: Message):
    ...
    await reply(message, f"✅ {args[0]} больше не админ")
...
async def cmd_subscription(message: Message):
    ...
    if args[0] == "on":
        ...
        await reply(message, "✅ Платный режим выключен — доступ у всех бесплатный.")
...
def _make_settext_handlers(key: str, command_name: str, prompt: str):
    ...
    async def receive_handler(message: Message):
        ...
        await reply(message, "✅ Текст обновлён")
