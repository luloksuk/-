...
🧩 <b>Драфт-помощник</b>
/addcounter — задать контры бойца: имя на первой строке, дальше через запятую кого он контрит
/delcounter Имя — удалить контры
/listcounters — список всех заданных контр
/setmaptypeheroes — задать список сильных бойцов для типа карты (Открытая/Средняя/Закрытая)
При /addmap после публикации бот спросит тип карты (3 кнопки)
Кнопка "Драфт" в меню запускает сессию: карта → 6 банов → вражеские пики по одному
...
/setsubactive, /setreminder, /setpaymentsuccess, /setreferral, /setpurchaseconfirm

🔘 <b>Названия кнопок меню</b>
/setbtnsearch, /setbtnsub, /setbtnref, /setbtnhelp, /setbtnpromo, /setbtndraft
...
async def start_draft_session(user_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            ...
            "VALUES (?, 'await_map', NULL, '[]', '[]') "
            "ON CONFLICT(user_id) DO UPDATE SET stage = 'await_map', map_type = NULL, "
            "banned = '[]', enemies = '[]'",
            (user_id,),
        )
        await db.commit()
...
def draft_controls_kb():
    kb = InlineKeyboardBuilder()
    kb.button(text="🔄 Новая игра", callback_data="draft:reset", style="primary")
    kb.button(text="❌ Выйти из драфта", callback_data="draft:exit", style="danger")
    kb.adjust(1)
    return kb.as_markup()
...
async def cb_draft_start(callback: CallbackQuery):
    ...
    await reply(
        callback,
        "🧩 Режим драфта включён.\nНапиши название карты (если её нет в базе — просто продолжим без учёта типа карты).",
        reply_markup=draft_controls_kb(),
    )
    await callback.answer()


@dp.callback_query(F.data == "draft:reset")
async def cb_draft_reset(callback: CallbackQuery):
    await start_draft_session(callback.from_user.id)
    await reply(
        callback,
        "🔄 Начинаем заново. Напиши название карты.",
        reply_markup=draft_controls_kb(),
    )
    await callback.answer()
...
async def handle_draft_message(message: Message):
    ...
    text = (message.text or "").strip()
    if not text:
        await reply(message, "Пришли текстом.", reply_markup=draft_controls_kb())
        return

    if stage == "await_map":
        map_entry = await find_entry("maps", text)
        map_type = map_entry["map_type"] if map_entry and map_entry["map_type"] else None
        await update_draft_session(message.from_user.id, stage="await_bans", map_type=map_type)
        if map_entry and map_type:
            note = f"Карта: {map_entry['name']} ({map_type})"
        elif map_entry:
            note = f"Карта: {map_entry['name']} (тип не задан — не учитываем)"
        else:
            note = "Такой карты нет в базе — продолжим без учёта типа карты."
        await reply(
            message,
            f"{note}\n\nПришли всех 6 забаненных бойцов — через запятую или каждого с новой строки.",
            reply_markup=draft_controls_kb(),
        )
        return
    ...
    if stage == "await_bans":
        ...
        if not banned:
            await reply(message, "Не понял список. Пришли имена через запятую.", reply_markup=draft_controls_kb())
            return
        await update_draft_session(message.from_user.id, stage="picks", banned=json.dumps(banned))
        await reply(
            message,
            f"✅ Баны приняты: {', '.join(banned)}\n\nТеперь пиши вражеских бойцов по одному, как только они пикают.",
            reply_markup=draft_controls_kb(),
        )
        return
    ...
    if _norm(text) in {_norm(b) for b in banned}:
        await reply(message, f"«{text}» забанен — враг не может его взять.", reply_markup=draft_controls_kb())
        return
    if len(enemies) >= 3:
        await reply(message, "Уже учтено 3 вражеских бойца — это максимум для 3v3. Жми «Новая игра», если начался новый матч.", reply_markup=draft_controls_kb())
        return
