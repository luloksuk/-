...
DEFAULT_DRAFT_PICK_PROMPT_TEXT = (
    "Теперь пиши вражеских бойцов по одному, как только они пикают.\n"
    "Свой пик отмечай знаком + в начале (например: +Шелли) — тогда учту твой состав."
)
DEFAULT_DRAFT_MAP_NOT_FOUND_TEXT = "Карта «{map}» не найдена в базе — пики по ней не покажу."
DEFAULT_DRAFT_BANS_NOT_RECOGNIZED_TEXT = (
    "⚠️ Баны не распознаны (пришли их со второй строки через запятую) — учитываю пустой список."
)
DEFAULT_DRAFT_MAP_TYPE_UNKNOWN_TEXT = "Тип карты неизвестен — ФП-рекомендации недоступны для этой карты."
DEFAULT_DRAFT_MAX_ENEMIES_TEXT = (
    "Уже учтено 3 вражеских бойца — это максимум для 3v3. Открой «🧩 Драфт» заново для новой игры."
)
DEFAULT_DRAFT_BANNED_PICK_TEXT = "«{hero}» забанен — враг не может его взять."
DEFAULT_DRAFT_ENEMY_TEAM_TEXT = "Вражеская команда: {enemies}"
DEFAULT_DRAFT_RECOMMEND_HEADER_TEXT = "🎯 Рекомендации:"
DEFAULT_DRAFT_FALLBACK_HEADER_TEXT = "⚠️ Чистого контрпика нет, вот компромиссные варианты:"
DEFAULT_DRAFT_NO_OPTIONS_TEXT = "Нет доступных вариантов (все забанены или уже взяты)."
...
EDITABLE_TEXTS = {
    ...
    "draft_map_not_found_text": (
        "setdraftmapnotfound",
        "Пришли новый текст, когда карта не найдена в базе. Можно использовать {map}.",
        DEFAULT_DRAFT_MAP_NOT_FOUND_TEXT,
    ),
    "draft_bans_not_recognized_text": (
        "setdraftbansnotfound",
        "Пришли новый текст, когда баны не удалось распознать.",
        DEFAULT_DRAFT_BANS_NOT_RECOGNIZED_TEXT,
    ),
    "draft_map_type_unknown_text": (
        "setdraftmaptypeunknown",
        "Пришли новый текст, когда тип карты неизвестен.",
        DEFAULT_DRAFT_MAP_TYPE_UNKNOWN_TEXT,
    ),
    "draft_max_enemies_text": (
        "setdraftmaxenemies",
        "Пришли новый текст, когда уже учтено 3 вражеских бойца.",
        DEFAULT_DRAFT_MAX_ENEMIES_TEXT,
    ),
    "draft_banned_pick_text": (
        "setdraftbannedpick",
        "Пришли новый текст, когда ввели забаненного бойца. Можно использовать {hero}.",
        DEFAULT_DRAFT_BANNED_PICK_TEXT,
    ),
    "draft_enemy_team_text": (
        "setdraftenemyteam",
        "Пришли новый текст-заголовок со списком вражеской команды. Можно использовать {enemies}.",
        DEFAULT_DRAFT_ENEMY_TEAM_TEXT,
    ),
    "draft_recommend_header_text": (
        "setdraftrecommendheader",
        "Пришли новый заголовок для рекомендаций контрпика.",
        DEFAULT_DRAFT_RECOMMEND_HEADER_TEXT,
    ),
    "draft_fallback_header_text": (
        "setdraftfallbackheader",
        "Пришли новый заголовок для случая, когда чистого контрпика нет.",
        DEFAULT_DRAFT_FALLBACK_HEADER_TEXT,
    ),
    "draft_no_options_text": (
        "setdraftnooptions",
        "Пришли новый текст на случай, когда нет доступных вариантов вообще.",
        DEFAULT_DRAFT_NO_OPTIONS_TEXT,
    ),
}
...
ADMIN_COMMANDS_HELP = """
🗺 <b>Карты и билды</b>
/addmap — добавить или обновить карту. Первая строка сообщения — название карты (по нему её ищут пользователи), всё что дальше — содержимое (пики); можно с форматированием, платными эмодзи и фото. После публикации бот попросит выбрать тип карты (Открытая/Средняя/Закрытая), а затем режим (Захват самоцветов, Брава Бол и т.д.) — оба параметра влияют на рекомендации в режиме "Драфт".
...
🧩 <b>Драфт-помощник</b>
/syncbrawlers — <b>начни с этого</b>. Автоматически загружает всех бойцов из BrawlAPI: имена, классы, гаджеты, звёздные силы. Заполняет базу за секунды вместо ручного ввода. Роли, заданные тобой вручную, не затираются.
/setalias — задать русские названия, чтобы поиск работал по ним. Формат строки: <code>El Primo = Эль Примо, Примо</code>. Без этого бот понимает только английские имена.
/bulkbrawlers — массово добавить/поправить бойцов вручную. Формат строки: <code>Имя: Роль, тир</code>. Роли: Дамагер, Танк, Снайпер, Тровер, Контроль, Ассасин, Саппорт. Тир — сила в мете 1-5 (по умолчанию 3). Нужен, чтобы проставить тиры и роли тем, у кого API не отдал класс.
/listbrawlers — список всех бойцов с ролями и тирами.
/rolerules — посмотреть правила «класс бьёт класс».
/setrolerules — заменить правила. Формат строки: <code>Ассасин → Снайпер</code>.
/addcounter — точечная контра для конкретной пары (имя на первой строке, дальше через запятую — кого он контрит). Всегда важнее правил классов. Сюда же пиши условия по гаджетам, например: <code>Эдгар (только с гаджетом на отталкивание)</code>.
/delcounter Имя — удалить контры бойца.
/listcounters — все заданные контры.
/addsynergy — с кем боец хорошо работает в связке (имя на первой строке, дальше список через запятую).
/setmaptypeheroes — бойцы, сильные на типе карты (Открытая/Средняя/Закрытая).
/setmodeheroes — бойцы, сильные в режиме (Захват самоцветов, Брава Бол, Нокаут, Охота за головами, Ограбление, Горячая зона).

Как работает драфт: пользователь жмёт «🧩 Драфт», присылает карту и баны одним сообщением (карта — первая строка, баны — со второй). Бот показывает пики по карте и лучших ФП. Дальше пользователь присылает вражеских бойцов по одному, а свои пики отмечает знаком + (например «+Шелли»). Алгоритм учитывает: прямые контры, правила классов, тип карты, режим, синергии, силу в мете, дубли ролей, пробелы в составе и уязвимость раннего пика — и объясняет, почему предлагает именно этих бойцов.
...
/setdraftmapnotfound — текст, когда карта не найдена в базе ({map})
/setdraftbansnotfound — текст, когда баны не распознаны
/setdraftmaptypeunknown — текст, когда тип карты неизвестен
/setdraftmaxenemies — текст при достижении лимита в 3 вражеских бойца
/setdraftbannedpick — текст, когда ввели забаненного бойца ({hero})
/setdraftenemyteam — заголовок со списком вражеской команды ({enemies})
/setdraftrecommendheader — заголовок над рекомендациями
/setdraftfallbackheader — заголовок, когда чистого контрпика нет
/setdraftnooptions — текст, когда вообще нет доступных вариантов
...
CREATE TABLE IF NOT EXISTS brawlers (
    ...
    name TEXT NOT NULL,
    role TEXT,
    meta_tier INTEGER NOT NULL DEFAULT 3,
    aliases TEXT,
    gadgets TEXT,
    star_powers TEXT
);
CREATE TABLE IF NOT EXISTS role_rules (
    ...
    PRIMARY KEY (attacker_role, victim_role)
);
CREATE TABLE IF NOT EXISTS mode_heroes (
...
async def init_db():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        ...
        await _ensure_column(db, "maps", "mode", "TEXT")
        await _ensure_column(db, "draft_sessions", "mode", "TEXT")
        await _ensure_column(db, "draft_sessions", "allies", "TEXT")
        await _ensure_column(db, "draft_sessions", "side", "TEXT")
        await _ensure_column(db, "brawlers", "aliases", "TEXT")
        await _ensure_column(db, "brawlers", "gadgets", "TEXT")
        await _ensure_column(db, "brawlers", "star_powers", "TEXT")
...
# Классы из BrawlAPI -> наши роли
API_CLASS_MAP = {
    "Damage Dealer": "Дамагер",
    "Tank": "Танк",
    "Marksman": "Снайпер",
    "Artillery": "Тровер",
    "Controller": "Контроль",
...
BRAWLERS_API_URL = "https://api.brawlapi.com/v1/brawlers"

# Дефолтные правила "класс бьёт класс" — используются, когда для конкретной
# пары бойцов не задана ручная контра через /addcounter.
DEFAULT_ROLE_RULES = [
    ("Ассасин", "Снайпер"),
    ("Ассасин", "Тровер"),
    ("Ассасин", "Саппорт"),
...
# Веса для подсчёта очков кандидата в драфте.
W_DIRECT_COUNTER = 3.0      # наш боец прямо контрит врага (ручная контра)
W_ROLE_COUNTER = 1.5        # контра выведена из правил классов
W_COUNTERED_BY = -4.0       # враг контрит нашего бойца (в драфте больнее всего)
W_ROLE_COUNTERED_BY = -2.0  # то же, но по правилам классов
W_MAP_TYPE = 1.2            # силён на этом типе карты
W_MODE = 1.5                # силён в этом режиме
W_SYNERGY = 1.0             # синергия с уже взятым союзником
W_META = 0.6                # множитель силы бойца в текущей мете
W_EXPOSURE = -1.0           # штраф за уязвимость (актуален на ранних пиках)
W_ROLE_GAP = 1.5            # закрывает недостающую роль в составе
...
async def set_brawler(name, role=None, meta_tier=None, aliases=None, gadgets=None, star_powers=None):
    key = name.strip().lower()
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM brawlers WHERE search_key = ?", (key,))
        row = await cur.fetchone()
        cur_role = row["role"] if row else None
        cur_tier = row["meta_tier"] if row else 3
        cur_aliases = row["aliases"] if row else None
        cur_gadgets = row["gadgets"] if row else None
        cur_sp = row["star_powers"] if row else None
        await db.execute(
            "INSERT INTO brawlers (search_key, name, role, meta_tier, aliases, gadgets, star_powers) "
            "VALUES (?, ?, ?, ?, ?, ?, ?) "
            "ON CONFLICT(search_key) DO UPDATE SET name = excluded.name, role = excluded.role, "
            "meta_tier = excluded.meta_tier, aliases = excluded.aliases, "
            "gadgets = excluded.gadgets, star_powers = excluded.star_powers",
            (
                key,
                name.strip(),
                role if role is not None else cur_role,
                meta_tier if meta_tier is not None else cur_tier,
                aliases if aliases is not None else cur_aliases,
                gadgets if gadgets is not None else cur_gadgets,
                star_powers if star_powers is not None else cur_sp,
            ),
        )
        await db.commit()


async def find_brawler_by_any_name(query):
    """Ищет бойца по основному имени или по одному из алиасов
    (русское название, сокращение и т.п.)."""
    q = query.strip().lower()
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM brawlers WHERE search_key = ?", (q,))
        row = await cur.fetchone()
        if row:
            return row
        cur = await db.execute("SELECT * FROM brawlers WHERE aliases IS NOT NULL AND aliases != ''")
...
async def set_brawler_aliases(name, aliases_list):
    row = await find_brawler_by_any_name(name)
    target = row["name"] if row else name
    joined = ", ".join(a.strip() for a in aliases_list if a.strip())
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "UPDATE brawlers SET aliases = ? WHERE search_key = ?", (joined, target.strip().lower())
        )
        await db.commit()
    return target


async def get_brawler(name):
    key = name.strip().lower()
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM brawlers WHERE search_key = ?", (key,))
...
async def list_all_brawlers():
    async with aiosqlite.connect(DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM brawlers ORDER BY name COLLATE NOCASE")
        return await cur.fetchall()


async def delete_brawler(name):
    key = name.strip().lower()
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute("DELETE FROM brawlers WHERE search_key = ?", (key,))
        await db.commit()


async def get_role_rules():
    """Возвращает множество пар (атакующая роль, жертва)."""
    async with aiosqlite.connect(DATABASE_PATH) as db:
        cur = await db.execute("SELECT attacker_role, victim_role FROM role_rules")
        rows = await cur.fetchall()
    if rows:
        return {(r[0], r[1]) for r in rows}
    return set(DEFAULT_ROLE_RULES)


async def set_role_rules(pairs):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute("DELETE FROM role_rules")
        for attacker, victim in pairs:
            await db.execute(
                "INSERT OR IGNORE INTO role_rules (attacker_role, victim_role) VALUES (?, ?)",
                (attacker, victim),
            )
        await db.commit()


async def set_mode_heroes(mode, heroes_list):
    joined = ", ".join(h.strip() for h in heroes_list if h.strip())
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO mode_heroes (mode, heroes) VALUES (?, ?) "
            "ON CONFLICT(mode) DO UPDATE SET heroes = excluded.heroes",
            (mode, joined),
        )
        await db.commit()


async def get_mode_heroes(mode):
    if not mode:
        return []
    async with aiosqlite.connect(DATABASE_PATH) as db:
        cur = await db.execute("SELECT heroes FROM mode_heroes WHERE mode = ?", (mode,))
        row = await cur.fetchone()
        if not row or not row[0]:
            return []
...
async def set_synergies(name, partners_list):
    key = name.strip().lower()
    joined = ", ".join(p.strip() for p in partners_list if p.strip())
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO synergies (search_key, name, partners) VALUES (?, ?, ?) "
            "ON CONFLICT(search_key) DO UPDATE SET name = excluded.name, partners = excluded.partners",
            (key, name.strip(), joined),
        )
        await db.commit()


async def get_synergies(name):
    key = name.strip().lower()
    async with aiosqlite.connect(DATABASE_PATH) as db:
        cur = await db.execute("SELECT partners FROM synergies WHERE search_key = ?", (key,))
        row = await cur.fetchone()
        if not row or not row[0]:
            return []
...
async def set_map_mode(search_key, mode):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute("UPDATE maps SET mode = ? WHERE search_key = ?", (mode, search_key))
        await db.commit()
...
async def start_draft_session(user_id):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        await db.execute(
            "INSERT INTO draft_sessions (user_id, stage, map_type, banned, enemies, mode, allies, side) "
            "VALUES (?, 'await_map_bans', NULL, '[]', '[]', NULL, '[]', NULL) "
            "ON CONFLICT(user_id) DO UPDATE SET stage = 'await_map_bans', map_type = NULL, "
            "banned = '[]', enemies = '[]', mode = NULL, allies = '[]', side = NULL",
            (user_id,),
        )
        await db.commit()
...
@dp.message(Command("syncbrawlers"), _is_admin_filter)
async def cmd_syncbrawlers(message: Message):
    await reply(message, "⏳ Загружаю ростер из BrawlAPI...")
    try:
        import aiohttp
        async with aiohttp.ClientSession() as session:
            async with session.get(BRAWLERS_API_URL, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                if resp.status != 200:
                    await reply(message, f"❌ API вернул статус {resp.status}. Попробуй позже.")
                    return
                data = await resp.json(content_type=None)
    except Exception as e:
        logging.exception("syncbrawlers failed")
        await reply(message, f"❌ Не удалось получить данные: {e}")
        return

    items = data.get("list") or []
    if not items:
        await reply(message, "❌ API вернул пустой список.")
        return
    ...
    for b in items:
        ...
        if not name:
            continue
        api_class = ((b.get("class") or {}).get("name") or "").strip()
        role = API_CLASS_MAP.get(api_class)
        gadgets = ", ".join(g.get("name", "") for g in (b.get("gadgets") or []) if g.get("name"))
        star_powers = ", ".join(s.get("name", "") for s in (b.get("starPowers") or []) if s.get("name"))

        existing = await get_brawler(name)
        # Роль из API не затираем, если админ уже задал её вручную
        role_to_set = role if role else (existing["role"] if existing else None)

        await set_brawler(
            name,
            role=role_to_set,
            gadgets=gadgets or None,
            star_powers=star_powers or None,
        )
        saved += 1
        if not role_to_set:
    ...
    note = ""
    if without_role:
        preview = ", ".join(without_role[:20])
        more = f" и ещё {len(without_role) - 20}" if len(without_role) > 20 else ""
        note = (
            f"\n\n⚠️ У {len(without_role)} бойцов в API не указан класс — задай роль вручную "
            f"через /bulkbrawlers:\n{preview}{more}"
        )

    await reply(
        message,
        f"✅ Загружено бойцов: {saved}\n"
        f"Имена сохранены на английском. Русские названия добавь через /setalias, "
        f"чтобы поиск работал по ним.{note}",
    )


@dp.message(Command("setalias"), _is_admin_filter)
async def cmd_setalias(message: Message):
    raw = message.text[len("/setalias"):].strip()
    if not raw:
        await set_pending_input(message.from_user.id, "args:setalias")
        await reply(
            message,
            "Пришли соответствия, каждое с новой строки:\n"
            "<code>English = Русское</code>\n\n"
            "Можно несколько вариантов через запятую:\n"
            "<code>El Primo = Эль Примо, Примо</code>\n"
            "<code>Piper = Пайпер</code>",
        )
        return
    await _process_setalias(message, raw)


async def _awaiting_setalias(message: Message):
    if not await is_admin(message.from_user.id):
        return False
    return await get_pending_input(message.from_user.id) == "args:setalias"


@dp.message(_awaiting_setalias)
async def receive_setalias(message: Message):
    await clear_pending_input(message.from_user.id)
    await _process_setalias(message, (message.text or "").strip())


async def _process_setalias(message: Message, raw: str):
    pairs = []
    unknown = []
    for line in raw.split("\n"):
        ...
        if not target or not aliases:
            continue
        row = await get_brawler(target)
        if not row:
            unknown.append(target)
            continue
        pairs.append((row["name"], aliases))

    if not pairs:
        msg = "Не нашёл ни одного бойца по этим именам."
        if unknown:
            msg += f"\nНе найдены: {', '.join(unknown[:15])}"
        await reply(message, msg)
        return

    await set_pending_action(message.from_user.id, "setalias", {"pairs": pairs})
    preview = "\n".join(f"{n} → {', '.join(a)}" for n, a in pairs[:20])
    more = f"\n…и ещё {len(pairs) - 20}" if len(pairs) > 20 else ""
    unknown_note = f"\n\n⚠️ Не найдены: {', '.join(unknown[:15])}" if unknown else ""
    await reply(
        message,
        f"🔎 Названий распознано: {len(pairs)}\n\n{preview}{more}{unknown_note}\n\nСохранить?",
        reply_markup=action_confirm_kb("✅ Сохранить", "❌ Отменить"),
    )


@register_apply("setalias")
async def _apply_setalias(admin_id, payload):
    for name, aliases in payload["pairs"]:
        await set_brawler_aliases(name, aliases)
    return f"✅ Названия сохранены для {len(payload['pairs'])} бойцов."


@dp.message(Command("bulkbrawlers"), _is_admin_filter)
async def cmd_bulkbrawlers(message: Message):
    raw = message.text[len("/bulkbrawlers"):].strip()
    if not raw:
        await set_pending_input(message.from_user.id, "args:bulkbrawlers")
        await reply(
            message,
            "Пришли список бойцов, каждый с новой строки, в формате:\n"
            "<code>Имя: роль, тир</code>\n\n"
            "Роли: " + ", ".join(BRAWLER_ROLES) + "\n"
            "Тир — сила в мете от 1 (слабый) до 5 (топ), можно не указывать (тогда 3).\n\n"
            "Например:\n"
            "Шелли: Шотган, 4\n"
            "Пайпер: Снайпер, 5\n"
            "Эль Примо: Танк",
        ...
        return
    await _process_bulkbrawlers(message, raw)


async def _awaiting_bulkbrawlers(message: Message):
    if not await is_admin(message.from_user.id):
        return False
    return await get_pending_input(message.from_user.id) == "args:bulkbrawlers"


@dp.message(_awaiting_bulkbrawlers)
async def receive_bulkbrawlers(message: Message):
    await clear_pending_input(message.from_user.id)
    await _process_bulkbrawlers(message, (message.text or "").strip())


async def _process_bulkbrawlers(message: Message, raw: str):
    parsed = []
    errors = []
    for line in raw.split("\n"):
        line = line.strip()
        ...
        role = bits[0] if bits and bits[0] else None
        tier = 3
        if len(bits) > 1 and bits[1].isdigit():
            tier = max(1, min(5, int(bits[1])))
        if role and role not in BRAWLER_ROLES:
            matched = next((r for r in BRAWLER_ROLES if r.lower() == role.lower()), None)
            if matched:
                role = matched
            else:
                errors.append(line)
                continue
        parsed.append((name.strip(), role, tier))

    if not parsed:
        await reply(message, "Не удалось разобрать ни одной строки. Проверь формат: Имя: Роль, тир")
        return

    await set_pending_action(
        message.from_user.id, "bulkbrawlers", {"items": parsed}
    )
    preview = "\n".join(f"{n} — {r or 'без роли'} (тир {t})" for n, r, t in parsed[:15])
    more = f"\n…и ещё {len(parsed) - 15}" if len(parsed) > 15 else ""
    err_note = f"\n\n⚠️ Не разобрал строк: {len(errors)}" if errors else ""
    await reply(
        message,
        f"🔎 Распознано бойцов: {len(parsed)}\n\n{preview}{more}{err_note}\n\nСохранить?",
        reply_markup=action_confirm_kb("✅ Сохранить", "❌ Отменить"),
    )


@register_apply("bulkbrawlers")
async def _apply_bulkbrawlers(admin_id, payload):
    for name, role, tier in payload["items"]:
        await set_brawler(name, role=role, meta_tier=tier)
    return f"✅ Сохранено бойцов: {len(payload['items'])}"


@dp.message(Command("listbrawlers"), _is_admin_filter)
async def cmd_listbrawlers(message: Message):
    rows = await list_all_brawlers()
    if not rows:
        await reply(message, "Пока не добавлено ни одного бойца. Используй /bulkbrawlers")
        return
    lines = [f"🥊 Бойцы в базе ({len(rows)}):\n"]
    for r in rows:
        lines.append(f"{r['name']} — {r['role'] or 'без роли'} (тир {r['meta_tier']})")
    text = "\n".join(lines)
    for i in range(0, len(text), 3800):
        await reply(message, text[i:i + 3800])


@dp.message(Command("setmodeheroes"), _is_admin_filter)
async def cmd_setmodeheroes(message: Message):
    kb = InlineKeyboardBuilder()
    for m in GAME_MODES:
        kb.button(text=m, callback_data=f"pickmode_admin:{m}", style="success")
    kb.adjust(1)
    await reply(message, "Для какого режима задаём список сильных бойцов?", reply_markup=kb.as_markup())


@dp.callback_query(F.data.startswith("pickmode_admin:"))
async def cb_pickmode_admin(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer()
        return
    mode = callback.data.split(":", 1)[1]
    current = await get_mode_heroes(mode)
    await set_pending_input(callback.from_user.id, f"args:modeheroes:{mode}")
    current_note = f"\n\nСейчас: {', '.join(current)}" if current else ""
    await reply(
        callback,
        f"Пришли список бойцов, сильных в режиме «{mode}», через запятую.{current_note}",
    )
    await callback.answer()


async def _awaiting_modeheroes(message: Message):
    if not await is_admin(message.from_user.id):
        return False
    pending = await get_pending_input(message.from_user.id)
    return bool(pending and pending.startswith("args:modeheroes:"))


@dp.message(_awaiting_modeheroes)
async def receive_modeheroes(message: Message):
    pending = await get_pending_input(message.from_user.id)
    mode = pending.split(":", 2)[2]
    await clear_pending_input(message.from_user.id)
    heroes_list = [h.strip() for h in (message.text or "").split(",") if h.strip()]
    if not heroes_list:
        await reply(message, "Нужен список через запятую. Попробуй снова: /setmodeheroes")
        return
    await set_pending_action(message.from_user.id, "setmodeheroes", {"mode": mode, "heroes": heroes_list})
    await reply(
        message,
        f"🔎 Сильные в режиме «{mode}»: {', '.join(heroes_list)}\n\nСохранить?",
        reply_markup=action_confirm_kb("✅ Сохранить", "❌ Отменить"),
    )


@register_apply("setmodeheroes")
async def _apply_setmodeheroes(admin_id, payload):
    await set_mode_heroes(payload["mode"], payload["heroes"])
    return f"✅ Список для режима «{payload['mode']}» сохранён."


@dp.message(Command("addsynergy"), _is_admin_filter)
async def cmd_addsynergy(message: Message):
    raw = message.text[len("/addsynergy"):].strip()
    if not raw:
        await reply(
            message,
            "Пришли одним сообщением:\nИмя бойца\nдальше — через запятую, с кем он хорошо работает",
        )
        return
    name, content = _split_first_line(raw)
    partners = [p.strip() for p in content.split(",") if p.strip()]
    if not name or not partners:
        await reply(message, "Нужно имя бойца на первой строке и список через запятую дальше.")
        return
    await set_pending_action(message.from_user.id, "addsynergy", {"name": name, "partners": partners})
    await reply(
        message,
        f"🔎 «{name}» хорошо работает с: {', '.join(partners)}\n\nСохранить?",
        reply_markup=action_confirm_kb("✅ Сохранить", "❌ Отменить"),
    )


@register_apply("addsynergy")
async def _apply_addsynergy(admin_id, payload):
    await set_synergies(payload["name"], payload["partners"])
    return f"✅ Синергии для «{payload['name']}» сохранены."


@dp.message(Command("rolerules"), _is_admin_filter)
async def cmd_rolerules(message: Message):
    rules = sorted(await get_role_rules())
    lines = ["⚔️ Правила «класс бьёт класс»:\n"]
    for attacker, victim in rules:
        lines.append(f"{attacker} → {victim}")
    lines.append(
        "\nЧтобы изменить — /setrolerules, дальше пришли список строк вида:\n"
        "<code>Ассасин → Снайпер</code>"
    )
    await reply(message, "\n".join(lines))


@dp.message(Command("setrolerules"), _is_admin_filter)
async def cmd_setrolerules(message: Message):
    await set_pending_input(message.from_user.id, "args:rolerules")
    await reply(
        message,
        "Пришли правила, каждое с новой строки, в формате:\n"
        "<code>Атакующий класс → Жертва</code>\n\n"
        "Классы: " + ", ".join(BRAWLER_ROLES),
    )


async def _awaiting_rolerules(message: Message):
    if not await is_admin(message.from_user.id):
        return False
    return await get_pending_input(message.from_user.id) == "args:rolerules"


@dp.message(_awaiting_rolerules)
async def receive_rolerules(message: Message):
    await clear_pending_input(message.from_user.id)
    pairs = []
    for line in (message.text or "").split("\n"):
        ...
        if len(parts) != 2:
            continue
        a, v = parts[0].strip(), parts[1].strip()
        a_m = next((r for r in BRAWLER_ROLES if r.lower() == a.lower()), None)
        v_m = next((r for r in BRAWLER_ROLES if r.lower() == v.lower()), None)
        if a_m and v_m:
            pairs.append((a_m, v_m))
    if not pairs:
        await reply(message, "Не разобрал ни одного правила. Формат: Ассасин → Снайпер")
        return
    await set_pending_action(message.from_user.id, "setrolerules", {"pairs": pairs})
    preview = "\n".join(f"{a} → {v}" for a, v in pairs[:20])
    await reply(
        message,
        f"🔎 Правил распознано: {len(pairs)}\n\n{preview}\n\nЗаменить текущие правила?",
        reply_markup=action_confirm_kb("✅ Заменить", "❌ Отменить"),
    )


@register_apply("setrolerules")
async def _apply_setrolerules(admin_id, payload):
    await set_role_rules([tuple(p) for p in payload["pairs"]])
    return f"✅ Правила обновлены ({len(payload['pairs'])} шт.)"
...
async def cb_pick_map_type(callback: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    for m in GAME_MODES:
        kb.button(text=m, callback_data=f"mapmode:{search_key}:{m}", style="success")
    kb.adjust(1)
    await callback.message.edit_text(
        f"✅ Тип карты «{row[0]}» — {map_type}.\n\nТеперь выбери режим:",
        reply_markup=kb.as_markup(),
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("mapmode:"))
async def cb_pick_map_mode(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer()
        return
    _, search_key, mode = callback.data.split(":", 2)
    async with aiosqlite.connect(DATABASE_PATH) as db:
        cur = await db.execute("SELECT name, map_type FROM maps WHERE search_key = ?", (search_key,))
        row = await cur.fetchone()
    if not row:
        await callback.message.edit_text("Карта не найдена (возможно, удалена).")
        await callback.answer()
        return
    await set_map_mode(search_key, mode)
    await callback.message.edit_text(f"✅ «{row[0]}» — {mode} · {row[1] or 'тип не задан'}")
    await callback.answer()
...
async def _build_draft_context():
    """Один раз подтягивает из базы всё, что нужно для расчёта, чтобы не
    дёргать её в цикле по каждому кандидату."""
    counters_rows = await list_all_counters()
    brawler_rows = await list_all_brawlers()

    counters_map_raw = {}
    for r in counters_rows:
        counters_map_raw[_norm(r["name"])] = [_norm(c) for c in r["counters"].split(",") if c.strip()]
    ...
    for r in brawler_rows:
        key = _norm(r["name"])
        roles[key] = r["role"]
        tiers[key] = r["meta_tier"] if r["meta_tier"] is not None else 3
        display[key] = r["name"]
        if r["aliases"]:
            for alias in r["aliases"].split(","):
                a = _norm(alias)
    ...
    for r in counters_rows:
        display.setdefault(_norm(r["name"]), r["name"])
    for name in await list_entry_names("builds"):
        display.setdefault(_norm(name), name)

    # Теперь, когда алиасы известны, приводим контры к каноническим ключам
    def _res(n):
        return alias_to_key.get(n, n)

    counters_map = {}
    for owner, victims in counters_map_raw.items():
        counters_map[_res(owner)] = {_res(v) for v in victims}
    ...
    return {
        ...
        "role_rules": await get_role_rules(),
    }


def _resolve(ctx, name):
    """Приводит введённое имя к каноническому ключу: русский алиас -> английское имя."""
    n = _norm(name)
    return ctx["alias_to_key"].get(n, n)


def _beats(ctx, attacker_norm, victim_norm):
    """Возвращает ('direct'|'role'|None) — как именно attacker бьёт victim.
    Ручная контра всегда приоритетнее правила классов."""
    if victim_norm in ctx["counters"].get(attacker_norm, set()):
        return "direct"
    a_role = ctx["roles"].get(attacker_norm)
    v_role = ctx["roles"].get(victim_norm)
    if a_role and v_role and (a_role, v_role) in ctx["role_rules"]:
        return "role"
    return None


async def _compute_draft_recommendation(
    enemies, banned, map_type, mode=None, allies=None, side=None
):
    """Считает топ-3 рекомендации с учётом контр, классов, режима, типа карты,
    своего состава, синергий, силы в мете и уязвимости пика."""
    allies = allies or []
    if not enemies and not allies:
        return "Пока не назван ни один вражеский боец."

    ctx = await _build_draft_context()
    banned_norm = {_resolve(ctx, b) for b in banned}
    enemies_norm = [_resolve(ctx, e) for e in enemies]
    allies_norm = [_resolve(ctx, a) for a in allies]
    taken = set(enemies_norm) | set(allies_norm) | banned_norm

    pool = set(ctx["display"].keys())
    map_strong = {_resolve(ctx, h) for h in (await get_map_type_heroes(map_type))} if map_type else set()
    mode_strong = {_resolve(ctx, h) for h in (await get_mode_heroes(mode))} if mode else set()

    # Каких ролей не хватает нашей команде
    ally_roles = {ctx["roles"].get(a) for a in allies_norm if ctx["roles"].get(a)}
    picks_made = len(allies_norm)
    picks_left = max(0, 3 - picks_made)
    is_early_pick = picks_made == 0

    candidates = []
    for hero_norm in pool:
        if hero_norm in taken:
            continue
        hero_name = ctx["display"][hero_norm]
        score = 0.0
        reasons = []
        beats_count = 0
        ...
        # 1. Контрим врагов
        beaten = []
        for e_norm in enemies_norm:
            kind = _beats(ctx, hero_norm, e_norm)
            if kind == "direct":
                score += W_DIRECT_COUNTER
                beats_count += 1
                beaten.append(ctx["display"].get(e_norm, e_norm))
            elif kind == "role":
                score += W_ROLE_COUNTER
                beats_count += 1
                beaten.append(ctx["display"].get(e_norm, e_norm))
        if beaten:
            reasons.append("контрит " + ", ".join(beaten))

        # 2. Нас контрят враги
        countered_by = []
        for e_norm in enemies_norm:
            kind = _beats(ctx, e_norm, hero_norm)
            if kind == "direct":
                score += W_COUNTERED_BY
                countered_by.append(ctx["display"].get(e_norm, e_norm))
            elif kind == "role":
                score += W_ROLE_COUNTERED_BY
                countered_by.append(ctx["display"].get(e_norm, e_norm))
        if countered_by:
            reasons.append("⚠️ проигрывает " + ", ".join(countered_by))

        # 3. Карта и режим
        if hero_norm in map_strong:
            score += W_MAP_TYPE
            reasons.append(f"силён на карте типа «{map_type}»")
        if hero_norm in mode_strong:
            score += W_MODE
            reasons.append(f"силён в режиме «{mode}»")

        # 4. Синергия с уже взятыми союзниками
        synergy_with = []
        hero_partners = {_resolve(ctx, p) for p in await get_synergies(hero_name)}
        for a_norm in allies_norm:
            if a_norm in hero_partners:
                synergy_with.append(ctx["display"].get(a_norm, a_norm))
            else:
                ally_partners = {_resolve(ctx, p) for p in await get_synergies(ctx["display"].get(a_norm, a_norm))}
                if hero_norm in ally_partners:
                    synergy_with.append(ctx["display"].get(a_norm, a_norm))
        if synergy_with:
            score += W_SYNERGY * len(synergy_with)
            reasons.append("синергия с " + ", ".join(synergy_with))

        # 5. Закрывает недостающую роль в составе
        hero_role = ctx["roles"].get(hero_norm)
        if hero_role and picks_made > 0:
            if hero_role in ally_roles:
                score -= 0.5
                reasons.append(f"дубль роли «{hero_role}»")
            else:
                score += W_ROLE_GAP
                reasons.append(f"закрывает роль «{hero_role}»")

        # 6. Сила в мете (1 — слабый, 5 — топ)
        tier = ctx["tiers"].get(hero_norm, 3)
        score += W_META * (tier - 3)
        if tier >= 5:
            reasons.append("топ меты")
        elif tier <= 1:
            reasons.append("слабый в мете")

        # 7. Уязвимость: сколько незабаненных бойцов могут его наказать.
        # Важно на раннем пике, когда враг ещё может ответить контрпиком.
        if is_early_pick and picks_left > 1:
            exposure = 0
            for other_norm in pool:
                if other_norm in taken:
                    continue
                if _beats(ctx, other_norm, hero_norm) == "direct":
                    exposure += 1
            if exposure:
                penalty = min(3.0, exposure * 0.3)
                score += W_EXPOSURE * penalty / 3.0
                if exposure >= 5:
                    reasons.append(f"легко наказать ({exposure} контр в пуле)")

        candidates.append((hero_name, round(score, 1), beats_count, reasons))

    strong = [c for c in candidates if c[2] >= 1] if enemies_norm else candidates
    fallback = bool(enemies_norm) and not strong
    ...
    lines = []
    if fallback:
        lines.append((await get_setting("draft_fallback_header_text")) + "\n")
    else:
        lines.append((await get_setting("draft_recommend_header_text")) + "\n")

    for i, (hero, score, beats_count, reasons) in enumerate(top, 1):
        sign = "+" if score >= 0 else ""
        why = "; ".join(reasons[:3]) if reasons else "нейтральный вариант"
        lines.append(f"{i}. <b>{hero}</b> ({sign}{score}) — {why}")

    if not top:
        lines.append(await get_setting("draft_no_options_text"))

    # Подсказка по составу
    if picks_made > 0 and picks_left > 0:
        missing = [r for r in BRAWLER_ROLES if r not in ally_roles]
        key_missing = [r for r in ("Танк", "Контроль", "Снайпер") if r in missing]
        if key_missing:
            lines.append(f"\n💡 В составе нет: {', '.join(key_missing)}")
...
async def handle_draft_message(message: Message):
    ...
    if stage == "await_map_bans":
        ...
        map_mode = map_entry["mode"] if map_entry and map_entry["mode"] else None
        await update_draft_session(
            message.from_user.id,
            stage="picks",
            map_type=map_type,
            mode=map_mode,
            banned=json.dumps(banned),
            allies=json.dumps([]),
        )

        parts = []
        if map_entry:
            head = f"🗺 <b>{map_entry['name']}</b>"
            meta_bits = [b for b in (map_mode, map_type) if b]
            if meta_bits:
                head += f" · {' · '.join(meta_bits)}"
            parts.append(f"{head}\n\n{map_entry['content']}")
        else:
            not_found_tpl = await get_setting("draft_map_not_found_text")
            parts.append(not_found_tpl.format(map=map_name))

        if banned:
            parts.append(f"\n🚫 Забанены: {', '.join(banned)}")
        else:
            parts.append("\n" + await get_setting("draft_bans_not_recognized_text"))

        # Рекомендации на первый пик — считаем полноценным алгоритмом,
        # без вражеских бойцов (учитываются карта, режим, мета, уязвимость).
        fp_header = await get_setting("draft_fp_header_text")
        if map_type or map_mode:
            fp_reco = await _compute_draft_recommendation(
                [], banned, map_type, mode=map_mode, allies=[], side=None
            )
            parts.append(f"\n{fp_header}\n{fp_reco}")
        else:
            parts.append("\n" + await get_setting("draft_map_type_unknown_text"))
    ...
    allies = json.loads(session["allies"] or "[]") if session["allies"] else []

    # Свой пик отмечается знаком "+" в начале: "+Шелли"
    is_ally = text.startswith("+")
    hero_name = text[1:].strip() if is_ally else text

    if not hero_name:
        await reply(message, "Пришли имя бойца.", reply_markup=await draft_controls_kb())
        return

    if _norm(hero_name) in {_norm(b) for b in banned}:
        banned_tpl = await get_setting("draft_banned_pick_text")
        await reply(message, banned_tpl.format(hero=hero_name), reply_markup=await draft_controls_kb())
        return
    ...
    if is_ally:
        if len(allies) >= 3:
            await reply(
                message,
                "У твоей команды уже 3 бойца — состав собран.",
                reply_markup=await draft_controls_kb(),
            )
            return
        allies.append(hero_name)
        await update_draft_session(message.from_user.id, allies=json.dumps(allies))
    else:
        if len(enemies) >= 3:
            await reply(
                message,
                await get_setting("draft_max_enemies_text"),
                reply_markup=await draft_controls_kb(),
            )
            return
        enemies.append(hero_name)
        await update_draft_session(message.from_user.id, enemies=json.dumps(enemies))

    recommendation = await _compute_draft_recommendation(
        enemies,
        banned,
        session["map_type"],
        mode=session["mode"],
    ...
    enemy_team_tpl = await get_setting("draft_enemy_team_text")
    enemy_team_line = enemy_team_tpl.format(enemies=", ".join(enemies) if enemies else "—")
    ally_line = f"\n🟢 Твоя команда: {', '.join(allies)}" if allies else ""
    await reply(
        message,
        f"{enemy_team_line}{ally_line}\n\n{recommendation}",
        reply_markup=await draft_controls_kb(),
    )
